...
| -------------------------- | --------------------- | -------------------------------------------------------------------- |
| `GATEWAY_PORT`               | 8080                  | Порт основного приложения / API шлюза.                               |
...
| `NOTIFICATION_SERVICE_PORT`            | 8085                  | Порт сервиса уведомлений.                |
...
| Переменная | Значение по умолчанию | Описание                                                                                                             |
| ---------- | --------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `KAFKA_EXTERNAL_PORT`      | 9094     | Порт для стандартного подключения к Kafka. |
...
- Инициирует аутентификацию пользователя по токену T-Invest, делая запрос в T-Connector и сохраняет зашифрованный токен в Redis.
...
- Долгий запрос с получением отчета по облигациям в PNG-формате, выполняется асинхронно через брокера сообщений Kafka. Gateway не ждет ответа и возвращает польователю сообщение о начале обработки запроса. 
...
- Долгий запрос с получением отчета по облигациям в PNG-формате возваращается пользователю через микросервис нотификаций(Notification-service). Взаимодействие между микросервисом нотификаций происходит через брокер сообщений Kafka
...
- Взаимодействие между сервисами осуществляется по HTTP и через брокер сообщений Kafka.
