...
func (s *Service) PrepareToGenerateTablePNG(ctx context.Context, generalBondReports *generalbondreport.GeneralBondReports, chatID int, accountID string) (_ []*domain.MediaGroup, err error) {
	...
	reports := make([][]generalbondreport.GeneralBondReportPosition, 0)
...
func sortGeneralBondReports(logger *slog.Logger, report map[generalbondreport.TickerTimeKey]generalbondreport.GeneralBondReportPosition) (_ []generalbondreport.GeneralBondReportPosition, err error) {
	...
	if len(report) == 0 {
		return []generalbondreport.GeneralBondReportPosition{}, domain.ErrEmptyReport
	}
