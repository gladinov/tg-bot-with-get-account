...
import (
	...
	factories "bonds-report-service/internal/infrastructure/tinkoffApi/testing"
	"bonds-report-service/internal/infrastructure/tinkoffApi/transport/mocks"
	"context"
	"encoding/json"
	"errors"
	"log/slog"
...
func TestGetAccounts(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		dtoResp := map[string]dto.Account{
			"acc1": {
				ID:   "acc1",
				Type: "broker",
				Name: "Main",
...
func TestGetOperations(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		want := factories.NewOperationDTO()
		respBody := []dto.Operation{want}
		...
		assert.NoError(t, err)
		assert.Len(t, result, 1)
		assert.Equal(t, want.OperationID, result[0].OperationID)
	})
